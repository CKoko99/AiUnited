...
const parentClasses = {
    ...
    image: {
        position: "relative",
        minHeight: { xs: "8rem", sm: "12rem", md: "15rem" },
        width: "100%",
    },
    contentContainer: {
        width: { xs: "100%", sm: "100%", md: "45%", lg: "45%" },
...
function ContentItem(props) {
    ...
    return <>
        ...
        >
            {props.title && <Typography sx={{ ...itemClasses.title }} fontWeight={"600"} variant="h5">{returnLocaleText(props.title)}</Typography>}
            {props.body && <Typography sx={{ ...itemClasses.body }} variant="h6">{returnLocaleText(props.body)}</Typography>}
        </Box>
    </>
}
                ...
                >
                    <Box sx={parentClasses.titles}>
                        {props.title && <Typography
                            ...
                            fontWeight={"bold"} variant="h3">{returnLocaleText(props.title)}</Typography>}
                        {props.subtitle && <Typography variant="h6">{returnLocaleText(props.subtitle)}</Typography>}
                    </Box>
                    {props.img &&
