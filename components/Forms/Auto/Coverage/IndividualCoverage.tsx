...
const TEXT = {
    selectCoverage: {
        en: "Select Your Coverage Option",
        es: "Seleccione su opción de cobertura",
    },
    validationError: {
        en: "Please select a coverage option",
        es: "Por favor seleccione una opción de cobertura",
    },
    policyCoverages: {
        en: "Liability Coverages",
        es: "Coberturas de Responsabilidad",
    },
    vehicleCoverages: {
        en: "Vehicle Coverages",
        es: "Coberturas del Vehículo",
    },
    additionalCoverages: {
        en: "Additional Coverages",
        es: "Coberturas Adicionales",
    },
    seeDetails: { en: "See More Details", es: "Ver Más Detalles" },
    coverageDetails: { en: "Coverage Details", es: "Detalles de la Cobertura" },
};
const policyCoverages = ["LiabilityBiLimit", "LiabilityPdLimit"];
const vehicleCoverages = ["ComprehensiveDeductible", "CollisionDeductible"];
...
function CoveragePackage(props: {
    ...
    defaultValues: { questionId: string; value: string | number | boolean }[];
    handleSelect: (
        index: number,
        values: { questionId: string; value: string | number | boolean }[],
    ) => void;
    index: number;
    selected: boolean;
    ...
    return (
        <>
            ...
            >
                <Box>
                    <Typography variant="h5" sx={{ ...classes.mainHeading }}>
                ...
                >
                    <Box>
                        ...
                        <Box sx={{ ...classes.showCoverageDetailsSection }}>
                            {policyCoverages.map(
                                (coverage, policyCoverageIndex) => {
                                    return (
                                        <Box
                                            key={coverage}
                                            sx={{
                                        ...
                                        >
                                            ...
                                            <Box
                                                sx={{
                                                    ...classes.showCoverageDetailLineValue,
                                                }}
                                            >
                                                <Typography variant="body1">
                                                    {returnLocaleText(
                    ...
                    <Box>
                        <Typography
                            ...
                            fontFamily={CustomFonts.Gustavo}
                        >
                            {returnLocaleText(TEXT.vehicleCoverages)}
                        </Typography>
                    ...
                    <Box>
                        <Typography
                            ...
                            fontFamily={CustomFonts.Gustavo}
                        >
                            {returnLocaleText(TEXT.additionalCoverages)}
                        </Typography>
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    <Typography variant="body1">
                                        {returnLocaleText(
                                            coverage.valueText as {
                                                [lang: string]: string;
                                            },
                                        )}
                                    </Typography>
                                    {coverage.key === "LiabilityBiLimit" && (
                                        <BodilyInjury inModal />
                                    )}
                                    {coverage.key === "LiabilityPdLimit" && (
                                        <PropertyDamage inModal />
                                    )}
                                    {coverage.key === "PipLimit" && (
                                        <PersonalInjuryProtection inModal />
                                    )}
                                    {coverage.key ===
                                        "ComprehensiveDeductible" && (
                                        <ComprehensiveDeductible inModal />
                                    )}
                                    {coverage.key === "CollisionDeductible" && (
                                        <CollisionDeductible inModal />
                                    )}
                                    {coverage.key === "MedPayLimit" && (
                                        <MedicalPayments inModal />
                                    )}
                                    {coverage.key ===
                                        "AccidentalDeathLimit" && (
                                        <AccidentalDeath inModal />
                                    )}
                                    {coverage.key ===
                                        "CustomEquipmentValue" && (
                                        <CustomEquipmentValue inModal />
                                    )}
                                    {coverage.key === "GapCoverage" && (
                                        <GapCoverage inModal />
                                    )}
                                    {coverage.key === "RentalLimit" && (
                                        <RentalLimit inModal />
                                    )}
                                    {coverage.key === "TowingLimit" && (
                                        <TowingLimit inModal />
                                    )}
                                    {coverage.key ===
                                        "UninsuredMotoristBiLimit" && (
                                        <UninsuredMotoristBI inModal />
                                    )}
                                    {coverage.key ===
                                        "UninsuredMotoristPdLimit" && (
                                        <UninsuredMotoristPD inModal />
                                    )}
                                </Box>
                            </Box>
                        );
...
function returnValueSelection(
    ...
    if (defaultSelections) {
        ...
        for (let i = 0; i < defaultSelections.length; i++) {
            let index = returnDefaultCoverages.findIndex(
                (cov) => cov.key === defaultSelections[i].questionId,
            );
...
function CustomCoverage(props: {
    ...
    const [coverageValues, setCoverageValues] = useState(
        returnValueSelection(props.coverages, props.defaultValues),
    );
    const selected = props.selected;
    ...
    function handleCoverageValueChange(
        coverageKey: string,
        value: string | number | boolean,
    ) {
        ...
        let coverageIndex = props.coverages.findIndex(
            (cov) => cov.key === coverageKey,
        );
        ...
        if (coverageKey === "ComprehensiveDeductible") {
            newCoverageValues[
                props.coverages.findIndex(
                    (cov) => cov.key === "CollisionDeductible",
                )
            ].value = value;
        }
        ...
        if (coverageKey === "CollisionDeductible") {
            newCoverageValues[
                props.coverages.findIndex(
                    (cov) => cov.key === "ComprehensiveDeductible",
                )
            ].value = value;
        }

        setCoverageValues(newCoverageValues);
        const passedCoverageValues = newCoverageValues.map(
            (coverage, index) => {
                return {
                    questionId: coverage.key,
                    value: coverage.value,
                ...
                >
                    <Box>
                        ...
                        <Box sx={{ ...classes.showCoverageDetailsSection }}>
                            {policyCoverages.map(
                                (coverage, policyCoverageIndex) => {
                                    let coverageOption = props.coverages.find(
                                        (cov) => cov.key === coverage,
                                    );
                                    let coverageIndex =
                                        props.coverages.findIndex(
                                            (cov) => cov.key === coverage,
...
export default function (props: {
    ...
    return (
        <>
            {props.custom ? (
                <CustomCoverage {...props} />
            ) : (
                <CoveragePackage {...props} />
            )}
        </>
    );
}
