    ...
    try {
        ...
        if (data.carrierResults) {
            ...
            filteredResults = filteredResults.filter(
                (result) =>
                    result.paymentOptions[0].numberOfPayments > 1 ||
                    result.term === 1,
            );
            ...
            const listReducedByCompanyList = reduceCompanysList(finalList, [
                "Alinsco",
                "Apollo",
                "Aguila Dorada",
                "Bluefire",
            ...
            listReducedByCompanyList.sort((a, b) => {
                if (
            ...
            const cheapestBuyNow = listReducedByCompanyList.find(
                (result) =>
                    (result[0] as { buyNowURL: string }).buyNowURL !== "",
            );
            const cheapestNotBuyNow = listReducedByCompanyList.find(
                (result) =>
                    (result[0] as { buyNowURL: string }).buyNowURL === "",
            );
        ...
        if (loading) {
            let interval = setInterval(() => {
                setLoadingPercent((prev) => {
                    setLoadingText(
                        LOADINGTEXT[
                            Math.floor(LOADINGTEXT.length * (prev / 100))
                        ],
                    );
            ...
            setTimeout(async () => {
                ...
                setTimeout(async () => {
                    try {
                        let secondResultsData = await getResults(props.id);
