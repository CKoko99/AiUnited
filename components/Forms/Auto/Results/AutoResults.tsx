...
const TEXT = {
  ...
  takeALook: {
    en: "You're Almost There – Let's Cross the Finish Line Together!",
    es: "¡Casi lo logramos! ¡Vamos a cruzar la línea de meta juntos!",
  },
  ...
  unfortunately: {
    en: "Oops! It seems like there was a hiccup on our end, and we couldn't retrieve quotes for you.",
    es: "¡Vaya! Parece que hubo un problema en nuestro sistema y no pudimos obtener cotizaciones para usted.",
  },
  sorry: {
    en: "Sorry for the inconvenience. Our dedicated team will assist you in getting a precise quote tailored to your needs.",
    es: "Lamentamos la molestia. Nuestro equipo dedicado lo ayudará a obtener una cotización precisa adaptada a sus necesidades.",
  },
  pleaseCall: {
    en: "Please call us to get a quote, or visit a nearby store.",
    es: "Llámenos para obtener una cotización o visite una tienda cercana.",
  },
...
function reduceCompanysList(BaseList, Companies) {
  ...
  for (let i = 0; i < Companies.length; i++) {
    let companyList = newBaseList.filter((result) =>
      result[0].productName.includes(Companies[i])
    );
    newBaseList = newBaseList.filter(
      (result) => !result[0].productName.includes(Companies[i])
    );
...
async function getResults(id) {
  try {
    ...
    //   console.log(resLink)
    const res = await fetch(resLink, { method: "GET" });
    ...
    if (data.carrierResults) {
      ...
      filteredResults = filteredResults.filter(
        (result) =>
          result.paymentOptions[0].numberOfPayments > 1 || result.term === 1
      );
      ...
      let groupedResults = filteredResults.reduce(
        (accumulator, currentValue) => {
          (accumulator[currentValue.carrierTransactionID] =
            accumulator[currentValue.carrierTransactionID] || []).push(
            currentValue
          );
          return accumulator;
        },
      ...
      finalList = reduceCompanysList(finalList, [
        "Alinsco",
        "Apollo",
        "Aguila Dorada",
        "Bluefire",
      ...
      finalList.sort((a, b) => {
        if (
          (a[0] as { buyNowURL: string }).buyNowURL !== "" &&
          (b[0] as { buyNowURL: string }).buyNowURL === ""
        ) {
          return -1;
        } else if (
          (a[0] as { buyNowURL: string }).buyNowURL === "" &&
      ...
      const cheapestBuyNow = finalList.find(
        (result) => (result[0] as { buyNowURL: string }).buyNowURL !== ""
      );
      const cheapestNotBuyNow = finalList.find(
        (result) => (result[0] as { buyNowURL: string }).buyNowURL === ""
      );
    ...
    if (loading) {
      let interval = setInterval(() => {
        setLoadingPercent((prev) => {
          setLoadingText(
            LOADINGTEXT[Math.floor(LOADINGTEXT.length * (prev / 100))]
          );
        ...
        setTimeout(async () => {
          ...
          try {
            ...
            setResults((oldResults) => {
              ...
              const length =
                oldResults.length > secondResultsData.length
                  ? oldResults.length
                  : secondResultsData.length;
            ...
            <>
              <Box>
                ...
                >
                  ...
                  <Typography variant="h6">{loadingPercent} %</Typography>
                  <Box
                    sx={{
                      margin: "auto",
                  ...
                  <>
                    <Box>
                      ...
                      >
                        ...
                        >
                          {returnLocaleText(TEXT.hello)} {props.name},
                        </Typography>
                        <Typography
                          variant="h3"
                          sx={{
                        ...
                        >
                          {returnLocaleText(TEXT.takeALook)}
                        </Typography>
                      </Box>
                      <Box
                        sx={{
                            ...
                            <ResultItem
                              ...
                              onClick={(conversionType: string) => {
                                ...
                                if (
                                  ...
                                  !clickedIndexArray.includes(conversionType)
                                ) {
