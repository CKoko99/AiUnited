...
const TEXT = {
    ...
    takeALook: {
        en: "Take a look at your personalized quotes!",
        es: "¡Eche un vistazo a sus cotizaciones personalizadas!",
    },
    ...
    unfortunately: {
        en: "Unfortunately we couldn't find any results for you online.",
        es: "Desafortunadamente no pudimos encontrar ningún resultado para usted en línea.",
    },
    pleaseCall: {
                ...
                >
                    {props.coverages.map((coverage, coverageIndex) => {
                        return (
                            ...
                            >
                                {COVERAGE_LIMITS.find(
                                    (cov) => cov[0] === coverage.name,
                                )?.[1] && (
                                    <>
...
export default function ContentItem(props: {
    ...
    let monthlyPayment = Number(
        ((totalPremium - dueToday) / numberOfPayments).toFixed(2),
    );
    if (numberOfPayments === 1) {
        monthlyPayment = 0;
        dueToday = Number(totalPremium.toFixed(2));
    }
            ...
            >
                ...
                >
                    <Typography
                        ...
                        textAlign={"center"}
                    >
                        {returnLocaleText(TEXT.term)}: {monthTerm}{" "}
                        {monthTerm > 1
                            ? returnLocaleText(TEXT.months)
                            : returnLocaleText(TEXT.month)}
                    </Typography>
                    <Typography variant="h6" fontFamily={CustomFonts.Gustavo}>
                        {returnLocaleText(TEXT.dueToday)}: ${dueToday}
                    </Typography>
                    <Typography fontFamily={CustomFonts.Gustavo} variant="h6">
                        {returnLocaleText(TEXT.monthlyPayment)}: $
                        {monthlyPayment}
                    </Typography>
                </Box>
                <Box
                ...
                >
                    ...
                    >
                        {returnLocaleText(TEXT.coverages)}
                    </Button>
                    <Button
                        sx={{
                            flex: 1,
